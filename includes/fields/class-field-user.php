...
class FieldForge_Field_User extends FieldForge_Field_Base {
	...
	public function render( int $post_id ): void {
		...
		$html .= '<input type="text" class="fieldforge-picker-search widefat" placeholder="' . esc_attr__( 'Search users…', 'ar-fieldforge' ) . '" autocomplete="off" />';
		...
		foreach ( $ids as $id ) {
			if ( '' !== $id && ( ! is_numeric( $id ) || (int) $id <= 0 ) ) {
				return sprintf(
					...
					__( '"%s" contains one or more invalid user IDs.', 'ar-fieldforge' ),
