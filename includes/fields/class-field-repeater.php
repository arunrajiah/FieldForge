...
class FieldForge_Field_Repeater extends FieldForge_Field_Base {
	...
	public function render( int $post_id ): void {
		...
		$button_label = esc_html( $this->field['button_label'] ?? __( 'Add Row', 'arunrajiah-fieldforge' ) );
		...
		if ( 'table' === $layout && ! empty( $sub_fields ) ) {
			...
			$html .= '<div class="fieldforge-repeater-header-cell fieldforge-repeater-order">' . esc_html__( 'Order', 'arunrajiah-fieldforge' ) . '</div>';
			$html .= '</div>';
		}
		...
		if ( ! empty( $rows ) ) {
			...
			$html .= '<p class="fieldforge-repeater-empty">' . esc_html__( 'No rows yet.', 'arunrajiah-fieldforge' ) . '</p>';
		}
	...
	private function render_row( int $row_index, array $row_data, array $sub_fields, FieldForge_Field_Registry $registry, string $name, string $layout ): string {
		...
		$html .= '<span class="fieldforge-repeater-drag dashicons dashicons-menu" title="' . esc_attr__( 'Drag to reorder', 'arunrajiah-fieldforge' ) . '"></span>';
		/* translators: %d: row number */
		$html .= '<span class="fieldforge-repeater-row-label">' . sprintf( esc_html__( 'Row %d', 'arunrajiah-fieldforge' ), $row_index + 1 ) . '</span>';
		$html .= '<div class="fieldforge-repeater-row-actions">';
		$toggle_title = esc_attr__( 'Collapse row', 'arunrajiah-fieldforge' );
		...
		$html .= '<button type="button" class="button fieldforge-repeater-remove-row">' . esc_html__( 'Remove', 'arunrajiah-fieldforge' ) . '</button>';
		$html .= '</div>';
		$html .= '</div>';
