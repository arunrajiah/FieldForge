...
class FieldForge_Field_Gallery extends FieldForge_Field_Base {
	...
	public function render( int $post_id ): void {
		...
		foreach ( $ids as $attachment_id ) {
			...
			$html .= '<button type="button" class="fieldforge-gallery-remove" aria-label="' . esc_attr__( 'Remove', 'fieldom' ) . '">&times;</button>';
			$html .= '</li>';
		}
		$html .= '</ul>';
		$html .= '<button type="button" class="button fieldforge-gallery-add">' . esc_html__( 'Add Images', 'fieldom' ) . '</button>';
		$html .= '</div>';
			...
			foreach ( $value as $id ) {
				if ( ! is_numeric( $id ) || (int) $id <= 0 ) {
					return sprintf(
						...
						__( '"%s" contains one or more invalid image IDs.', 'fieldom' ),
