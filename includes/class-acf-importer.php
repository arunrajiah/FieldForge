...
class FieldForge_ACF_Importer {
	...
	public function import( string $json ) {
		...
		if ( null === $data ) {
			return new WP_Error( 'invalid_json', __( 'Invalid JSON provided.', 'ar-fieldforge' ) );
		}
		...
		if ( ! is_array( $data ) ) {
			return new WP_Error( 'invalid_format', __( 'Unexpected JSON structure.', 'ar-fieldforge' ) );
		}
	...
	private function import_group( array $acf ) {
		if ( empty( $acf['key'] ) || empty( $acf['title'] ) ) {
			return new WP_Error( 'missing_fields', __( 'ACF group missing key or title.', 'ar-fieldforge' ) );
		}
	...
	private function convert_field( array $acf ): ?array {
		...
		switch ( $ff_type ) {
			...
			case 'repeater':
				...
				$field['button_label'] = sanitize_text_field( $acf['button_label'] ?? __( 'Add Row', 'ar-fieldforge' ) );
				break;
			...
			case 'flexible_content':
				...
				$field['min']          = (int) ( $acf['min'] ?? 0 );
				$field['max']          = (int) ( $acf['max'] ?? 0 );
				$field['button_label'] = sanitize_text_field( $acf['button_label'] ?? __( 'Add Layout', 'ar-fieldforge' ) );
				break;
		}
	...
	public function ajax_import(): void {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( array( 'message' => __( 'Insufficient permissions.', 'ar-fieldforge' ) ), 403 );
		}
