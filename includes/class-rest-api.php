...
class FieldForge_REST_API {
	...
	public function get_options_page_fields( WP_REST_Request $request ) {
		...
		if ( ! $page ) {
			return new WP_Error( 'fieldforge_options_not_found', __( 'Options page not found.', 'fieldom' ), array( 'status' => 404 ) );
		}
	...
	public function update_options_page_fields( WP_REST_Request $request ) {
		...
		if ( ! is_array( $body ) ) {
			return new WP_Error( 'fieldforge_options_invalid', __( 'Request body must be a JSON object.', 'fieldom' ), array( 'status' => 400 ) );
		}
	...
	public function update_fields_for_post( $value, WP_Post $post, string $attr ) {
		if ( ! current_user_can( 'edit_post', $post->ID ) ) {
			return new WP_Error(
				...
				__( 'You do not have permission to edit this post.', 'fieldom' ),
				array( 'status' => 403 )
			);
		}
		...
		if ( ! is_array( $value ) ) {
			return new WP_Error(
				...
				__( 'fieldforge_fields must be an object.', 'fieldom' ),
				array( 'status' => 400 )
			);
		}
