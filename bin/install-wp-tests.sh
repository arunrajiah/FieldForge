...
#
# Adapted from the official WordPress plugin boilerplate.
...
create_db() {
    mysql -u "$DB_USER" --password="$DB_PASS" -h "${DB_HOST%%:*}" --port="${DB_HOST##*:}" -e "CREATE DATABASE IF NOT EXISTS $DB_NAME;" 2>/dev/null || true
}
